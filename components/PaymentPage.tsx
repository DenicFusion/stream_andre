...
export const PaymentPage: React.FC<PaymentPageProps> = ({ userData, onSuccess, onBack }) => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'TRANSFER') {
        if (timeLeft > 0) {
            ...
            // We don't force back immediately to let them read the alert, but button is disabled via logic
        }
    }
  ...
  const verifyTransactionWithGroq = async () => {
    if (!proofText.trim()) {
        showAlert("Input Required", "Please paste your transaction receipt text or debit alert SMS.", "error");
        return;
    }
    ...
    if (!GROQ_API_KEY) {
        // Fallback simulation if no key is provided, so the app doesn't break during testing
        // In production, this should block.
        setAiResponseMsg("Groq API Key missing. Simulating verification...");
        setTimeout(() => {
            setAiVerified(true);
            setAiResponseMsg("Verified (Simulation Mode)");
        }, 2000);
        return;
    }
    ...
    try {
        // Anti-Fraud: Check if this specific proof was used before in this browser
        const usedProofs = JSON.parse(localStorage.getItem('stream_used_proofs') || '[]');
        const proofHash = btoa(proofText.slice(0, 50)); // Simple hash of start of text
        
        if (usedProofs.includes(proofHash)) {
             throw new Error("Fraud Detected: This payment proof has already been used.");
        }

        const selectedBank = BANK_DETAILS[selectedBankIndex];
        
        const systemPrompt = `
            You are a strict Payment Verification AI for Stream Africa.
            Your job is to analyze the user's pasted transaction text (SMS or Receipt) and confirm if it matches the current session requirements.
            
            Session Requirements:
            1. Amount must correspond to roughly 12,000 NGN (allow variance like 12000.00).
            2. Destination Bank must match: ${selectedBank.bankName}.
            3. Destination Account must match (partial match ok): ${selectedBank.accountNumber}.
            4. The text must look like a valid debit alert or receipt.
            
            Return ONLY a JSON object: { "verified": boolean, "reason": "short explanation" }.
        `;
        ...
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                messages: [
                    ...
                    { role: "user", content: `Analyze this text: "${proofText}"` }
                ],
        ...
        if (result.verified) {
            setAiVerified(true);
            setAiResponseMsg("Payment Confirmed by AI System ✅");
        ...
        showAlert("Verification Error", error.message || "AI service unreachable.", "error");
        setAiVerified(false);
    } finally {
  ...
  const handleTransferDone = () => {
      if (!aiVerified && GROQ_API_KEY) {
          showAlert("Verification Required", "Please verify your payment with the AI system first.", "error");
          return;
      }
      
      const bank = BANK_DETAILS[selectedBankIndex];
      const bankInfo = `${bank.bankName} (${bank.accountNumber})`;
      // For transfers, we send the proof text as reference if available, or empty
      onSuccess(proofText.slice(0, 20) + '...', bankInfo);
  };
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <div className="border border-gray-200 rounded-xl p-4 bg-gray-50">
                        ...
                        <p className="text-xs text-gray-500 mb-3">
                            Paste your Transaction Ref, Debit Alert SMS, or Narration below. The AI will verify it instantly.
                        </p>
                        
                        <textarea 
                            value={proofText}
                            onChange={(e) => setProofText(e.target.value)}
                            placeholder="Example: Debit Amt: NGN12,000.00 Desc: TRF/StreamAfrica..."
                            className="w-full p-3 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent min-h-[80px] mb-3"
                            disabled={timeLeft <= 0 || aiVerified}
                        ></textarea>

                        {aiResponseMsg && (
                            <div className={`text-xs font-bold mb-3 p-2 rounded ${aiVerified ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                {aiResponseMsg}
                            </div>
                        )}
                        ...
                        <Button 
                            ...
                            fullWidth 
                            className={`py-3 text-sm !bg-gray-800 hover:!bg-black mb-1`}
                            disabled={isVerifying || aiVerified || timeLeft <= 0}
                        >
                            {isVerifying ? 'AI Analysis in Progress...' : aiVerified ? 'Verified Successfully' : 'Verify Payment with AI'}
                        </Button>
                    </div>
